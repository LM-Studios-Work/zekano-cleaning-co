...
const categoryImage = "/cleaning images/zenako-house-cleaning-johannesburg.webp"

export default async function ResidentialCleaningHub() {
  const payload = await getPayload({ config: configPromise })
  const [{ docs: catImgs }, detailImageMap] = await Promise.all([
    payload.find({
      collection: 'service-category-images',
      where: { category: { equals: 'residential' } },
      limit: 1,
    }),
